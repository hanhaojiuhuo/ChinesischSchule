...
export default function PrivacyPage() {
  ...
  return (
    <>
      ...
      {expandedField && (
        <ExpandModal
          label={`Privacy – ${expandedField.lang.toUpperCase()} ${expandedField.field}`}
          value={
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="font-cn text-3xl font-bold text-school-dark mb-2">
              {isAdmin ? (
                <input
                  type="text"
                  value={draft.zh.privacy.pageTitle}
                  onChange={(e) => draft.updZh("privacy", { pageTitle: e.target.value })}
            ...
            <p className="text-lg text-gray-500">
              {isAdmin ? (
                <input
                  type="text"
                  value={draft.de.privacy.pageTitle}
                  onChange={(e) => draft.updDe("privacy", { pageTitle: e.target.value })}
                ...
                <>
                  {draft.de.privacy.pageTitle}{showEn("privacy") && ` · ${draft.en.privacy.pageTitle}`}
                </>
              )}
            </p>
          </div>
          ...
          <div className={`bg-white rounded-xl shadow-sm border p-8 space-y-10 text-sm text-gray-700 leading-relaxed ${isAdmin ? "border-amber-400 ring-2 ring-amber-400 ring-offset-2" : "border-school-border"}`}>
            ...
            <section>
              ...
              {isAdmin ? (
                <div>
                  ...
                  <EditArea
                    value={draft.de.privacy.content}
                    onChange={(v) => draft.updDe("privacy", { content: v })}
                    className="min-h-[200px]"
                  />
                </div>
              ) : (
                <div className="whitespace-pre-line">{draft.de.privacy.content}</div>
              )}
            </section>
            ...
            {showEn("privacy") && (
              <section className="border-t border-gray-100 pt-8">
                <h2 className="text-base font-bold text-school-dark mb-3 pb-1 border-b border-gray-100">
                  {draft.en.privacy.pageTitle}
                </h2>
                <div className="whitespace-pre-line">{draft.en.privacy.content}</div>
              </section>
            )}
          </div>
